...
suite('AgentHostChatContribution', () => {
	...
	suite('permission requests', () => {
		...
		test('permission_request owned by another client shows shared confirmation and can approve', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables);
			...
			const invocation = toolInvocations[toolInvocations.length - 1];
			assert.ok(invocation, 'Should have received an observed tool confirmation item');
			...
			assert.ok(agentHostService.dispatchedActions.some(
				a => a.action.type === ActionType.SessionToolCallConfirmed
					&& (a.action as IToolCallConfirmedAction).toolCallId === 'tc-other-client'
					&& (a.action as IToolCallConfirmedAction).approved === true
			));
		...
		test('permission_request approved from another surface clears local confirmation without redispatch', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService } = createContribution(disposables);
			...
			fire({
				...
				session,
				turnId,
				toolCallId: 'tc-external-approve',
				toolName: 'runTask',
				displayName: 'Run Task',
				toolClientId: 'another-window',
			} as SessionAction);
			...
			assert.strictEqual(agentHostService.dispatchedActions.some(
				a => a.action.type === ActionType.SessionToolCallConfirmed && (a.action as IToolCallConfirmedAction).toolCallId === 'tc-external-approve'
			), false);
