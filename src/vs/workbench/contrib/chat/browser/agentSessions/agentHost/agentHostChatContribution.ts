...
export class AgentHostContribution extends Disposable implements IWorkbenchContribution {
	...
	private _registerAgent(agent: AgentInfo): void {
		...
		const agentRegistration = store.add(this._activeClientService.registerForAgent(sessionType, agent.provider));
	...
	private _autoAuthenticatedAgents(agents: readonly AgentInfo[]): readonly AgentInfo[] {
		return agents.filter(agent => !this._shouldSuppressDirectorProtectedResourceAuth(agent));
	}

	private _canAutoListAgentSessions(agent: AgentInfo): boolean {
		return !this._shouldSuppressDirectorProtectedResourceAuth(agent);
	}

	private _shouldSuppressDirectorProtectedResourceAuth(agent: AgentInfo): boolean {
		return this._configurationService.getValue<boolean>(AgentHostDirectorAgentEnabledSettingId)
			&& agent.provider === 'director'
