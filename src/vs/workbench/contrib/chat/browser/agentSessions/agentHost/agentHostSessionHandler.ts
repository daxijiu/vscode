...
import { agentHostClientToolReferenceNamesForProvider, IAgentHostActiveClientService } from './agentHostActiveClientService.js';
...
export { agentHostClientToolReferenceNamesForProvider, toolDataToDefinition };
...
export class AgentHostSessionHandler extends Disposable implements IChatSessionContentProvider {
	...
	constructor(
		...
		const clientTools = this._activeClientService.getClientTools(this._config.provider);
		this._register(autorun(reader => {
			const defs = clientTools.read(reader);
	...
	private _getCurrentActiveClient(): SessionActiveClient {
		return this._activeClientService.getActiveClient(this._config.sessionType, this._config.connection.clientId, this._config.provider);
	}
	...
	private _setupMarkdownPart(
		...
		store.add(autorun(reader => {
			...
			const markdown = rawMarkdownToString(delta, this._config.connectionAuthority);
			// supportHtml is load bearing. Without it, chatModel may merge
			// thinking markdown into adjacent edit parts.
			markdown.supportHtml = true;
			opts.sink([{ kind: 'markdownContent', content: markdown }]);
		}));
	}
