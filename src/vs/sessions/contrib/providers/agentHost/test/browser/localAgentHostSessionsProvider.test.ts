...
import { AgentHostDirectorAgentEnabledSettingId, AgentSession, IAgentHostService, type IAgentCreateSessionConfig, type IAgentListSessionsOptions, type IAgentSessionMetadata } from '../../../../../../platform/agentHost/common/agentService.js';
...
class MockAgentHostService extends mock<IAgentHostService>() {
	...
	override async listSessions(options: IAgentListSessionsOptions = {}): Promise<IAgentSessionMetadata[]> {
		...
		return [...this._sessions.values()].filter(session => !options.provider || AgentSession.provider(session.session) === options.provider);
	}
...
suite('LocalAgentHostSessionsProvider', () => {
	...
	test('Director enablement only suppresses auto-listing for Director required-auth sessions', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		const config = new TestConfigurationService();
		await config.setUserConfiguration(AgentHostDirectorAgentEnabledSettingId, true);
		...
		agentHost.setAgents([
			{ provider: 'copilotcli', displayName: 'Copilot', description: '', models: [], protectedResources } as AgentInfo,
			{ provider: 'director', displayName: 'Director', description: '', models: [], protectedResources } as AgentInfo,
		]);
		agentHost.addSession(createSession('copilot-auth', { provider: 'copilotcli', summary: 'Copilot' }));
		agentHost.addSession(createSession('director-auth', { provider: 'director', summary: 'Director' }));
		...
		assert.deepStrictEqual(agentHost.listSessionsCalls.map(call => call.provider), ['copilotcli']);
		assert.deepStrictEqual({
			eventCount: changes.length,
			cachedTitles: provider.getSessions().map(s => s.title.get()),
		}, {
			eventCount: 1,
			cachedTitles: ['Copilot'],
		});
	}));
